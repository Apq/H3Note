...
## 十六、0x4E32E0 的 score 比较字段与类型（新确认）

### 1. 候选条目是 `int + int` 的 8 字节结构

`0x4E32E0` / `0x469C50` 的候选条目布局已经更明确：

```text
Cand:
  +0x00  int score/value
  +0x04  int target/附加字段
```

证据：

- 生成候选时，局部变量被写成两个 DWORD：
  - `mov dword [ebp - 0x1c], eax`
  - `mov dword [ebp - 0x18], ecx`
- 最终插入容器时成对写入：
  - `mov dword [edi], ecx`
  - `mov dword [edi + 4], edx`
- 所有搬移、右移、遍历都按 `8` 字节步长进行。

### 2. score 是整数，不是浮点

从 `0x4E32E0` 的评分/插入路径看：

- 进入函数时，候选的主要数值经由 `int` 寄存器传递；
- 排序插入使用 `cmp` / `jl` / `jge` 这种整数比较；
- 结果写回也是 `mov dword`；
- 看不到对候选 score 的 `fild/fstp/fmul` 之类浮点保存。

因此当前应把候选 score 记为：**32 位有符号整数 int**。
...
- 先确定插入点；
- 把 `[insert_pos .. end)` 整体右移 8 字节；
- 再写入新条目；
- 末尾的有效边界通过 `add dword [esi + 8], 8` 增长。

结合比较和插入方向，当前可以确认容器是按**升序**维护的；因此：

- **收益最大目标 = 尾项 / 最后一个元素**
- 不需要再对整个容器做一次线性 `max` 扫描

### 4. 关键比较点

在当前已读范围里，还没有看到 `0x4E32E0` 主体直接解出“比较的是 `[+0]` 还是 `[+4]`”。

但可以确认：

- 比较函数最终决定的是“是否左移/右移”；
- 参与排序的条目是 8 字节整条复制；
- 这套逻辑更像“按 score 排好序的候选表”，而不是只存一个 best 值。

### 5. 当前可用结论

```text
数据类型：int (32-bit signed)
条目结构：{ int score; int target; }
排序方式：升序插入
最大收益目标：容器尾项（最后一个条目）
```
...
- `score` 与 `target` 两个 DWORD 的确切语义映射；
- 具体是 `0x4E32E0` 还是 `0x469C50` 的哪一处在计算 score；
- 是否有额外权重在 `0x469F30` 之类函数里再做一次修正。
