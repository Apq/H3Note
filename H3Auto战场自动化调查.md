...
> **2026-07-14 更新**：第 2 节"插件如何向战斗系统提交动作"的推测已被实测反编译结论取代。
> 动作提交路径、action code、控制权判定的完整结论见 **《战争机器控制权逆向笔记.md》**。
> 结论要点：动作统一从 `battle+0x3c(action)/+0x40/+0x44(target)/+0x48` 读取，由 `FUN_004786b0 @ 0x004786b0` 执行；
> 控制权分流在 `FUN_00474520 @ 0x00474520`（返回 0=人工，1=自动）。本节以下内容保留为历史推测记录。
