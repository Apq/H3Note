...
当前实现使用 `0x495C50`（`Hook_CycleCombatScreen`，战斗动画循环）。

早期测试时发现在该位置画的绿色方块能显示、但黄色边框不显示，误判为"太早被覆盖"。

根因是 backbuffer 在 HD/OpenGL 模式下为 BGRA 格式，直接写入像素颜色不对。

**解决方案**：背景图解码为 RGB565 `_Pcx16_`，文字叠加后一次性 blt 到 backbuffer，再用 `H3Redraw` 强制刷新。
这样在 `0x495C50` 画面板就能正常显示，无需换到 `0x600430`。

| 地址 | 类型 | 说明 |
|------|------|------|
| `0x495C50` | HiHook | 战斗动画循环（当前使用）✓ |
| `0x600430` | LoHook | backbuffer Blt 完成后（已废弃，不再使用） |

### 2.3 当前渲染方案

完全不使用 DirectDraw，所有绘制走游戏原生 `screen_pcx16`（16bpp RGB565 surface），最后用 `H3Redraw` 强制刷新。

核心流程：
```
PCX (8bpp indexed) → RGB565 _Pcx16_ (bg_)
